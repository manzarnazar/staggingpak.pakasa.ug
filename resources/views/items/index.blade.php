...
@section('script')
    ...
    if (value && typeof value === 'string') {
        let fullUrl = value.startsWith('http') ? value : window.location.origin + '/' + value;
        return `<img src="${fullUrl}" class="img-fluid" width="50"> <br> ${fullUrl}`;
    }
    return 'No Image';
}
    ...
    console.log("Gallery Images Debug:", value); // Debugging output
    ...
    try {
        // ✅ If value is an array of objects, extract the `image` field
        if (Array.isArray(value)) {
            return value.map(imgObj => {
                let img = imgObj.image || imgObj; // Handle both objects and strings
                let fullUrl = typeof img === 'string' ? 
                    (img.startsWith('http') ? img : window.location.origin + '/' + img) : 'Invalid Image';
                
                return `<a href="${fullUrl}" target="_blank">
                            <img src="${fullUrl}" class="img-fluid" width="50"> 
                        </a> <br> 
                        <a href="${fullUrl}" target="_blank">${fullUrl}</a>`;
            }).join('<br>');
        }

        // ✅ If value is a string but contains JSON, parse it
        if (typeof value === 'string') {
            let parsedImages = JSON.parse(value);
            if (Array.isArray(parsedImages)) {
                return parsedImages.map(img => {
                    let fullUrl = img.startsWith('http') ? img : window.location.origin + '/' + img;
